...
class MessagePassingLayer(nn.Module):
    ...
    def forward(self, f, s, edge_index, edge_f=None, edge_s=None):
        if edge_index.shape[1] == 0:
            ...
            _f = torch.cat((f[edge_index[0]], f[edge_index[1]]), dim=-1) #[M, 3, 2F]
            ...
            _f_T = _f.transpose(-1, -2) #[M, 2F+Fe, 3]
...
class MessagePassingNetwork(nn.Module):
    def __init__(self, n_layer, p_step, node_f_dim, node_s_dim, edge_f_dim, edge_s_dim, hidden_dim,
        ...
        for i in range(self.n_layer):
            self.networks.append(MessagePassingLayer(node_f_dim, node_s_dim, edge_f_dim,
                                                           edge_s_dim, hidden_dim, activation))
        self.edge_readout = edge_readout
        if edge_readout:
            self.readout = EdgeReadoutLayer(node_f_dim, node_s_dim, edge_f_dim, edge_s_dim, hidden_dim,
