...
from .utils import get_fully_connected, MessagePassingNetwork
from torch_scatter import scatter

class CoupledGNN(nn.Module):
    def __init__(self, n_layer, p_step, s_dim, hidden_dim=128, activation=nn.SiLU(), cutoff=0.10, gravity_axis=None):
        super(CoupledGNN, self).__init__()
        self.cutoff = cutoff
        ...
        self.gamma = 1.0
        self.eta = 0.001
        self.local_net = MessagePassingNetwork(n_layer=n_layer, p_step=p_step,
                                                           node_f_dim=3, node_s_dim=hidden_dim,
                                                           edge_f_dim=1, edge_s_dim=0, hidden_dim=hidden_dim,
                                                           activation=activation, edge_readout=True)
        self.global_net = MessagePassingNetwork(n_layer=n_layer, p_step=p_step,
        ...
        self.spread_net = MessagePassingNetwork(n_layer=n_layer, p_step=p_step,
    ...
    def build_graph(self, x_p):
        ...
        self.n_relation = edge_index.shape[1] # number of edges
        return edge_index

    def type_filter(self, x_p, f_p, s_p, obj_id, obj_type, num_obj):
    ...
    def forward(self, f_p, h_p, f_o, obj_id, obj_type, n_num=None, o_num=None):
        """
        x_p: [node_num, 3]; v_p [node_num, 3]; h_p[node_num, 4];
        obj_id: [node_num]; obj_type: [obj_num]
        """        
        x_p, v_p = f_p[..., :3], f_p[..., 3:]
        try:
            h_p[x_p[..., 1] < 0.1, -1] = 1
        ...
        f_o_p = scatter(torch.stack((x_p, v_p), dim=-1), obj_id, dim=0, reduce='mean')  # [N_obj, 3, 2]
        ...
        edge_attr_f, edge_attr_s = self.local_net(f_p, s_p, edge_index_inter, edge_attr_inter_f)
        #  [M_out, 3, x], [M_out, H] || inter message-passing
        ...
        edge_index_o, _ = get_fully_connected(num_obj, device=obj_id.device, loop=True)  # [2, M_obj]
        ...
        count = scatter(edge_pseudo, edge_mapping, dim=0, reduce='sum', dim_size=num_obj ** 2) # [N_obj **2]
        mask = count > 0
        edge_index_o, edge_attr_o_f, edge_attr_o_s = edge_index_o[..., mask], edge_attr_o_f[mask], edge_attr_o_s[mask]
        f_o_, s_o_ = self.global_net(f_o[..., 1:], s_o, edge_index_o, edge_attr_o_f, edge_attr_o_s)  # [N_obj, 3, 2]
        ...
        f_p_, s_p_ = self.spread_net(f_p_, s_p_, edge_index_inner, edge_attr_inner_f)  # [N, 3, x], [N, H]
        
        dx_p = self.type_filter(x_p, f_p_, s_p_, obj_id, obj_type, num_obj)  # [N, 3]
        dv_p = dx_p * self.gamma - v_p
        df_p = torch.cat((dx_p, v_p), dim=-1)  # [N, 3, 2]
        df_o = f_o_.reshape(-1,6) - f_o
        return df_p, df_o